...
1. you can run different versions of the same command, using the syntax @version
...
When the connection is successfully established, the openevent is fired.

Listen for it by assigning a callback function to the onopenproperty of the connection object:
...
[Here](https://glitch.com/edit/#!/flavio-websockets-server-example) is a live example of a WebSockets server.

[Here](https://glitch.com/edit/#!/flavio-websockets-client-example) is a WebSockets client that interacts with the server.
...
You can find more about [flags](https://nodejs.org/api/fs.html#fs_file_system_flags).
...
I published a full [article](https://flaviocopes.com/node-event-emitter/) on that, so here I will just describe the API without further examples on how to use it.
...
Points to the global instance of the Agent object, which is an instance of the http.Agentclass.
...
If the table has a primary key with auto_increment, the value of that will be returned in the results.insertIdvalue:
...
Node.js assumes it’s always running in a development environment. You can signal Node.js that you are running in production by setting the NODE_ENV=productionenvironment variable.
...
For example [Pug](https://pugjs.org/api/express.html), the templating library used by Express, compiles in debug mode if NODE_ENV is not set to production. Express views are compiled in every request in development mode, while in production they are cached. There are many more examples.
